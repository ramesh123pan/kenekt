...
export default async function handler(req, res) {
  ...
  try {
    ...
    const propertyData = await page.evaluate(() => {
      ...
      const priceValue = priceDisplay
        ? Number(priceDisplay.replace(/[^0-9.-]+/g, ""))
        : null;

      const features = Array.from(
        document.querySelectorAll("ul.inline-flex li")
      ...
      const description =
        document.querySelector(".lexxy-content")?.innerText.trim() || null;
      const floorplanImg =
        document.querySelector("#floorplan img")?.src || null;
      ...
      const extractListByHeading = (headingText) => {
        ...
        const heading = headings.find(
          (h) =>
            h.innerText &&
            h.innerText.trim().toLowerCase() === headingText.toLowerCase()
        );
        if (!heading) return [];

        let wrapper =
          heading.closest("div.relative") ||
        ...
        if (!ul) {
          ...
          while (sibling && !sibling.querySelector("ul")) {
      ...
      const locationData = Array.from(
        document.querySelectorAll('[data-map-target="listings"] > div')
      ).map((el) => ({
